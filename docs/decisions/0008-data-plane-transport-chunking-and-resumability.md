...
### 9. Transfer-session authentication — ACCEPTED: short-lived, transfer-scoped signed capability over HTTPS (Issue #15)
...
1. **Data-plane transport is HTTPS, not plain HTTP.** ADR-0008 point 1's "HTTP-based data plane" did not itself decide HTTP vs. HTTPS. Given the provisioning network is never a trust anchor (`AGENTS.md`), any bearer-style authorization material would otherwise be exposed to capture/replay in plaintext, and Artifact contents may themselves carry sensitive user/customer data, M0/V1 requires HTTPS for the data plane. Server identity reuses the **same pinned Server TLS certificate/fingerprint** already authenticated for the Agent Protocol WSS connection via trusted bootstrap (ADR-0010/ADR-0011) — no second trust relationship is introduced. The Agent does not present a client certificate for the data plane, exactly as it does not for Agent Protocol (ADR-0005) — **mTLS is rejected** as disproportionate for a threat model already served by pinned Server TLS plus a scoped bearer capability.
2. **Authorization is a short-lived, transfer-scoped, Server-signed bearer capability** ("Candidate A" in the evaluated alternatives), bound to exactly one `transfer_id`, `artifact_id`, `endpoint_id`, direction, and the `attempt_id` of the transfer JobStep's Attempt that caused it to be issued. It is **not** derived from, or a synonym for, the Endpoint's long-lived Agent runtime credential ("Candidate B" — rejected, see "Alternatives considered"): the capability grants authority over exactly one transfer, never generic data-plane access.
3. **Issuance rides the already-authenticated Agent Protocol control-plane channel**, via two new, strictly additive message types (`docs/specifications/m0-agent-protocol-contract.md` "Transfer authorization"): `TransferAuthorizationRequest{transfer_id}` (Agent → Server) and either `TransferAuthorizationGrant{transfer_id, token, expires_at}` or `TransferAuthorizationDenied{transfer_id, reason}` (Server → Agent). This addition does **not** reopen WSS, pinned TLS, `AuthRequest`/`SessionEstablished`, or `BootstrapEvidence` — it follows the exact precedent already set when `BootstrapEvidence` was added additively (Issue #13). Bulk Artifact bytes still never flow over Agent Protocol — only the small authorization capability does, exactly as `ActionDispatch` parameters already carry action-type-specific data without becoming a bulk-transfer channel.
4. **The token is self-verifying (cryptographically signed by a Server-held signing secret) and is never separately persisted as a durable, reusable secret.** Every use — every chunk request and every renewal request — is additionally revalidated against current **durable** transfer/Attempt/credential state (transfer not terminal, Endpoint/direction/artifact match, Endpoint credential still `CredentialActive`). This combination gives real-time revocation effectiveness (denying a cancelled transfer or a revoked credential immediately, even before the token's own expiry) without requiring a persisted token blacklist or session table.
5. **Authorization lifetime is decoupled from durable transfer identity.** The token is short-lived and renewable; `transfer_id`, the chunk manifest, and already-verified chunks (point 6) are wholly unaffected by token expiry or renewal — a transfer never restarts, and no chunk is ever re-verified, merely because its authorization secret expired. Renewal reuses the same `TransferAuthorizationRequest`/`Grant` pair for the same, still-legitimate (non-terminal) `transfer_id`, independent of JobStep/Attempt retry (ADR-0006) — renewal is not a retry and never creates a new Attempt.
6. **A transient WSS control-plane disconnect does not revoke an already-issued, still-valid data-plane token** — the HTTPS data-plane channel does not depend on the WSS socket remaining open. It also never becomes an indefinitely reusable independent access channel: it remains short-lived, scoped to one transfer, and revalidated against durable state on every use, and an **explicit** Endpoint credential revocation (`CredentialRevoked`) cascades to deny further use of any outstanding token for that Endpoint, even before the token's own expiry.

Exact token TTL, concrete signature/wire format, and HTTP-level details (header names, status codes) remain implementation-time, consistent with this ADR's existing pattern for `digest_algorithm` and chunk size (points 3–4). The full operational contract — bindings, issuance sequence, revocation/fail-closed cases, reconnect/restart behavior, and Simulator scenarios — is defined in `docs/specifications/m0-data-plane-and-storage-contracts.md` "Transfer-session authentication", consistent with how points 1–8 above already split decision (here) from detailed contract (there).
...
- **A per-transfer asymmetric keypair / client-certificate (mTLS-style) capability ("Candidate C", point 9)**: considered and rejected as disproportionate complexity — the Server is both sole issuer and sole verifier of transfer authorization, so a symmetric Server-held signing secret achieves the same integrity/authenticity property as an asymmetric scheme without introducing a PKI M0 does not otherwise need.
- **Plain HTTP for the data plane**: rejected (point 9) — would expose the transfer-scoped bearer capability, and potentially sensitive Artifact content, to any device with provisioning-network position, which is explicitly never a trust anchor.
...
- Transfer-session authentication (point 9) is resolved: `docs/specifications/m0-agent-protocol-contract.md` gains two new, strictly additive message types (`TransferAuthorizationRequest`, `TransferAuthorizationGrant`/`TransferAuthorizationDenied`), and `docs/specifications/m0-data-plane-and-storage-contracts.md` gains the full operational contract — no future implementation Work Package inherits this as a hidden architectural decision.
- The M0 data plane requires HTTPS, reusing the Server TLS identity already pinned via trusted bootstrap; no separate data-plane PKI or mTLS is introduced.
- Issue #7's Simulator must exercise the real `TransferAuthorizationRequest`/`Grant`/`Denied` messages and real per-request revalidation, not a faked authorization boundary, consistent with the Simulator's already-accepted real-transport fidelity rule (`docs/specifications/m0-simulator-contract-and-validation-strategy.md`).
