...
**This is an architectural expectation, not yet an empirically validated result.** The M0 target workload — 20–24 concurrent simulated endpoints producing this durable-write pattern — has **not** been measured against SQLite's actual write contention, latency, or backpressure behavior. The M0 architecture/contract baseline defines this validation obligation and its acceptance scenario (`docs/specifications/m0-simulator-contract-and-validation-strategy.md` "Persistence-load validation"); the first post-M0 implementation vertical slice executes the empirical measurement against that representative workload. If that measurement shows unacceptable write contention, latency, or backpressure at the M0 target, this ADR must be revisited — SQLite's acceptance here is conditional on that future evidence, not a substitute for it, though the empirical measurement itself is not a precondition for closing the M0 architecture/contract baseline. This ADR does not define what "unacceptable" means in concrete performance thresholds; that determination belongs to the vertical slice's validation work, informed by observed behavior rather than a number fixed in advance.
...
- The M0 architecture/contract baseline (`docs/specifications/m0-architecture-baseline.md`) defines the persistence-load validation obligation and its acceptance scenario (`docs/specifications/m0-persistence-observability-and-domain-events.md` "Validation expectations", `docs/specifications/m0-simulator-contract-and-validation-strategy.md` "Persistence-load validation"): representative persistence load at the M0 20–24 concurrent-endpoint target, measuring actual durable write volume, contention, latency, and backpressure. The first post-M0 implementation vertical slice **must** execute that empirical measurement — this ADR's SQLite acceptance is conditional on that evidence, not a substitute for it. This measurement is not a precondition for closing the M0 architecture/contract baseline itself; the absence of an implementation during M0 is expected, not a gap.
- If that measurement shows unacceptable write contention, latency, or backpressure at the M0 target, this ADR must be revisited — this is a first-class revisit trigger, alongside the concrete triggers listed in "Repository port/adapter boundary" (higher concurrent-write pressure, remote operation, multiple Server instances, HA, multi-site). A general sense that Bamep "might get bigger" is not by itself a trigger.
