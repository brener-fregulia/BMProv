...
The current enrollment token (`crates/domain/src/credential.rs`,
`enrollment::issue`) already embeds a cryptographically random 16-byte nonce
inside its signed payload. The implementation may expose or reuse that
existing value as `boot_context_id` rather than introducing a second random
value. Exact encoding remains implementation-time.
...
Server process restart. Conceptually it holds at least:
...
The signed expiry inside an unresolved E1/E2 governs only whether that
boot-scoped credential may perform its **first** successful redemption. Before
promotion, admission is governed by enrollment HMAC/signature verification
plus the signed expiry. After successful promotion, the credential is a
...
The promoted predecessor may therefore remain valid beyond the enrollment
token's original signed expiry when the persisted credential-chain validity
window extends further. This is intentional:
...
4. If no persisted credential mapping exists **and** the credential is an
   enrollment credential: verify its enrollment authenticity and original
   signed expiry, resolve and lock its `BootContext`, and continue through
   first-contact / genuine-reboot resolution (ADR-0004; ADR-0012 point 1).
...
### 11. Enrollment signing-key restart requirement

Enrollment credential verifier/signing material required to validate
credentials still within their unresolved signed validity window must remain
stable across normal Server process restarts. Otherwise a durable
`BootContext` alone cannot provide restart-safe redemption: a regenerated
signing key would reject an otherwise still-valid, not-yet-promoted E1/E2
after a restart, defeating the purpose of persisting `BootContext` at all.

The concrete key-storage mechanism, rotation policy, backup/recovery
handling, and secret-management implementation are **not** designed by this
ADR and remain separate implementation/security design work.
...
- `docs/specifications/m0-endpoint-identity-lifecycle.md` is amended to define
  ...
  unresolved-enrollment `BootContext` lookup, promotion into the normal
  credential chain, and the promoted-predecessor expiry semantics (point 6)
  in the "Credential chain, rotation, and revocation" subsection it already
  owns. That Specification remains the normative lifecycle definition; this
  ADR records the decision and its reasoning.
...
- Enrollment signing-key storage, rotation, backup/recovery, and
  secret-management implementation (point 11) are explicitly deferred and not
  designed here.
