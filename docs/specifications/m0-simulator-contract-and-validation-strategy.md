...
`docs/specifications/m0-persistence-observability-and-domain-events.md` requires, per ADR-0007, that the post-M0 first implementation vertical slice exercise representative persistence load at the M0 20–24 concurrent-endpoint target, measuring actual durable write volume, contention, latency, and backpressure against the expectation in ADR-0007, and states this as a requirement on the scenario this Specification defines. This obligation is part of post-M0 empirical validation, not a precondition for closing the M0 architecture/contract baseline: M0 defines the required validation obligation and acceptance scenario below; the first post-M0 implementation vertical slice executes the empirical measurement; unacceptable observed SQLite behavior requires ADR-0007 to be revisited; the absence of an implementation during M0 is expected, not a gap to fill with invented numbers.
