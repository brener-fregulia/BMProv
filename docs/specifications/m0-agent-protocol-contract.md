...
Every successful `AuthRequest` results in the Server issuing a fresh runtime credential, delivered atomically as part of `SessionEstablished{protocol_version, session_id, runtime_credential, credential_expires_at}` — never as a separate message. `runtime_credential` is opaque from this protocol's perspective; its internal representation is owned by `docs/specifications/m0-endpoint-identity-lifecycle.md` and ADR-0012, not defined here. `credential_expires_at` follows the timestamp convention in "Wire encoding" below.
...
3. The internal structure/format of `bootstrap_assertion`, and the site trust-anchor provisioning mechanism it depends on — owned by `docs/specifications/m0-trusted-bootstrap-and-server-fingerprint-contract.md` (Issue #13), `Approved`; sub-problem (B) is accepted (ADR-0011). Not decided here.
